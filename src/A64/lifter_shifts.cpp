...
llvm::Value *Lifter::PerformShift(Instance& rinst, llvm::Value *value, aarch64_shifter type, Value *shift) {
    ...
    case AArch64_SFT_LSL:
        return rinst.builder->CreateShl(value, shift);
    case AArch64_SFT_MSL:
        return rinst.builder->CreateSub(rinst.builder->CreateShl(shift, value),
                                        shift); // TODO: Verify
    case AArch64_SFT_LSR:
        return rinst.builder->CreateLShr(value, shift);
    case AArch64_SFT_ASR:
        return rinst.builder->CreateAShr(value, shift);
    case AArch64_SFT_ROR:
        return rinst.builder->CreateOr(
            rinst.builder->CreateLShr(value, shift),
            rinst.builder->CreateShl(value, rinst.builder->CreateSub(ConstantInt::get(value->getType(), value->getType()->getIntegerBitWidth()), shift)));
    default:
        DYNAUTIC_ASSERT(!"Invalid shift operation");
    }
}
...
uint64_t Lifter::PerformShift(uint64_t value, uint8_t bits, aarch64_shifter type, uint8_t shift) {
    ...
    case AArch64_SFT_LSL:
        return value << shift;
    case AArch64_SFT_MSL:
        return (shift << value) - shift; // TODO: Verify
    case AArch64_SFT_LSR:
        return value >> shift;
    case AArch64_SFT_ASR: {

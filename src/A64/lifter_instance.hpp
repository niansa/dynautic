...
private:
    struct Branch {
        ...
        Branch(Lifter::Instance& parent, const llvm::Twine& name) { basic_block = llvm::BasicBlock::Create(*parent.context, name, parent.func); }
...
public:
    ...
    bool IsDynamicBranch() const { return branch->dynamic; }
    VAddr GetBranchOrigin() const { return branch->origin; }
    VAddr GetBranchAddr() const { return branch->addr; }
    VAddr *GetBranchAddrPtr() { return &branch->addr; }
    ...
    llvm::Type *GetIntVectorType(uint8_t bits, uint8_t elements) { return llvm::FixedVectorType::get(GetIntType(bits), elements); }
